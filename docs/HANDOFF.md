...
| extension | `extension/**`, `src/app/api/jargon/**`, `src/app/jargon/**`, `src/lib/{cors,ratelimit}.ts` | MV3 overlay (Shadow DOM, service-worker fetch, popup language selector), public explain/escalate API, in-app PDF reader with the same overlay (`src/components/jargon/**`, pdf.js canvas + text layer, practice form from `scripts/make-practice-form.ts`). |
